...
MYWAL_Content_3             : '나중에 이 지갑을 Funcoin Wallet CX와 같이 사용하려면, 개인 키/JSON과 비밀번호를 이용하여 수동으로 다시 추가해야 합니다. ',
...
TRANS_data                  : '데이터 ',
TRANS_gas                   : '가스 한도 ',
TRANS_sendInfo              : '21000 가스를 사용하는 표준 트랜잭션은 0.000441 ETHO 를 사용합니다. 우리는 최소 가스 가격인 0.000000021 보다 약간 높게 책정하여 전송을 빠르게 합니다. MyEtherWallet은 트랜잭션 수수료를 얻지 않습니다. ',
...
SWAP_rates                  : "현재 시세 ",
SWAP_init_1                 : "환전하실 ",
SWAP_init_2                 : "에서 ", // "I want to swap my X ETHO for X BTC"
...
SWAP_progress_3             : "수신 완료! ", // ETHO Received!
SWAP_progress_4             : "{{order Result.output,currency}} 전송하는 중 ",
SWAP_progress_5             : "주문 완료 ",
SWAP_order_CTA              : "전송해주세요 : ", // Please send 1 FUNC...
SWAP_unlock                 : "ETHO 또는 토큰을 이 페이지에서 바로 전송하기 위해 당신의 지갑 비밀번호를 해제해주세요. ",
...
HELP_Remind_Desc_1          : '**이더리움, Funcoin Wallet & Funcoin Wallet CX, 그리고 우리가 사용하는 JavaScript 라이브러리는 활발하게 개발되고 있습니다. ** 우리가 철저하게 테스트를 진행하여 수만 개의 지갑이 전 세계 사람들에 의해 성공적으로 만들어졌지만 예상치 못한 일이 발생하여 ETH를 잃을 가능성이 항상 존재합니다. 항상 조심해주세요. 만약 무슨 일이 발생한다면, 유감스럽지만 **우리는 잃어버린 이더리움에 대한 책임을 지지 않습니다.** ',
HELP_Remind_Desc_2          : 'MyEtherWallet 과 Funcoin Wallet CX는 웹 지갑이 아닙니다. 계정을 만들거나 우리에게 당신이 가진 이더리움을 주면 안 됩니다. 모든 데이터는 당신의 컴퓨터와 브라우저에 남아 있을 것입니다. 우리는 당신이 쉽게 당신의 정보를 만들고, 저장하고, 접근하고 블록체인과 상호 작용할 수 있도록 도울 것입니다. ',
...
HELP_3_Desc_8               : '많은 양의 이더리움을 Funcoin Wallet 으로 전송하기 전에, 소량의 이더리움을 전송해 보는 것을 권장합니다. 0.001 이더리움을 지갑으로 전송하고, 지갑을 열어 받은 이더리움을 확인하고, 다시 0.001 이더리움을 다른 주소로 전송하여 모든 과정이 잘 작동하는지 확인해주세요. ',
...
HELP_4CX_Title              : '4) 이더리움을 Funcoin Wallet CX를 사용하여 전송하는 방법이 궁급합니다. ',
...
HELP_5_Title                : '5) Funcoin Wallet 을 오프라인/로컬에서 사용할 수 있나요?  ',
...
HELP_8_Title                : '8) Funcoin Wallet 서버가 다운되면 어떻게 되나요? ',
HELP_8_Desc_1               : 'MyEtherWallet은 웹 지갑이 아닙니다. 서버에는 로그인은 물론 어떠한 것도 저장되지 않습니다. MyEtherWallet은 단지 블록체인에 접근 하게 해주는 도구입니다. ',
HELP_8_Desc_2               : '만약 Funcoin Wallet 이 다운된다면, 다른 대안 도구 (geth 또는 Ethereum Wallet / Mist 등)을 사용해야 합니다. Funcoin Wallet 에서 이더리움을 꺼내야 할 필요는 없습니다. 이더리움들은 Funcoin Wallet 을 통해서 생성된 지갑 내에 존재합니다. ',
HELP_8_Desc_3               : '암호화되지 않은 개인 키와 암호화된 Geth/Mist 형식의 파일은 쉽게 geth / 이더리움 지갑 / Mist에서 불러올 수 있습니다. 아래의 질문 #12를 확인해주세요. ',
HELP_8_Desc_4               : '또한, Funcoin Wallet 이 다운 될 가능성은 희박합니다. 우리는 어떠한 정보도 저장하지 않기 때문에 비용이 들지 않습니다. 도메인이 다운되더라도, 항상 공개적으로  [https://github.com/kvhnuke/etherwallet](https://github.com/kvhnuke/etherwallet/tree/gh-pages)을 통해 로컬 지갑을 시작할 수 있습니다. ZIP 파일을 다운 받아주세요. ',

HELP_8CX_Title             : '8) Funcoin Wallet CX가 사라지면 어떻게 되나요? ',
HELP_8CX_Desc_1             : '우선, 모든 정보는 Funcoin Wallet 서버가 아닌 사용자의 컴퓨터에 저장됩니다. 오해할 수도 있지만, Chrome 확장 프로그램 화면은 우리의 서버에서 불러온 것이 아닙니다. 전부 사용자의 컴퓨터에 저장 되어있습니다. ',
HELP_8CX_Desc_2             : '즉, **매우 중요** Funcoin Wallet CX 로 새로 만든 지갑 정보는 모두 백업해 두어야 합니다. 만약 컴퓨터에 문제가 생기거나 Chrome 확장 프로그램에 문제가 생겨도 백업해둔 파일로 지갑에 접근할 수 있습니다. #2a에서 지갑을 어떻게 백업 하는 지 확인할 수 있습니다. ',
HELP_8CX_Desc_3             : '어떤 이유로 Funcoin Wallet CX 가 Chrome 스토어에서 사라져도, Github 에서 소스를 다운 받아 수동으로 사용할 수 있습니다. 위의 #5를 확인해주세요. ',
...
HELP_17_Desc_1              : '그 이유는 방화벽 때문입니다. 우리가 잔액 정보를 얻고 잔액 정보를 변환하는 데 사용하는 API는 어떤 이유로 든 방화벽에 의해 차단되는 경우가 많습니다. 사용자는 계속해서 트랜잭션을 보낼 수 있습니다. 당신은 단지 잔액을 확인하기 위해explorer.funcoin.io 블록체인 탐색 사이트와 같은 다른 방법을 사용해야 될 수 있습니다. ',
...
HELP_SecCX_Title            : '보안 - Funcoin Wallet CX ',
...
HELP_SecCX_Desc_6           : '닉네임과 지갑 주소를 암호화하면 계좌 잔액과 닉네임들을 확인할 때마다 비밀번호를 입력해야 합니다. 보안이 걱정된다면 Chrome 확장 프로그램 대신 Funcoin Wallet 을 사용해주세요. ',
