...
ENS_Helper_4                    :  'そのドメイン名に対して可能な最大の支払ETHを決めてください(自分の<u>Bid Amount</u>)。その口座に、入札に必要な額と0.01ETHのガス消費分が残っていることを確かめてください。', /*Decide the maximum amount of ETHO you are willing to pay for the name (your <u>Bid Amount</u>). Ensure that account has enough to cover your bid + 0.01 ETHO for gas.*/
...
SWAP_rates                  : "現在のレート ",
SWAP_init_1                 : "両替する ",
SWAP_init_2                 : " を ", // "I want to swap my X ETHO for X BTC"
...
tx_notFound_4               : '右上の下展開メニューから、別のノードを選択してください。 (例： `ETHO (Etherscan.io)` or `ETHO (Infura.io)` or `ETHO (MyEtherWallet)`) から選んで再度確認する。',
...
TXFEE_Desc                  : 'この処理料金は自分の処理をブロックに配置するためにマイナーに支払われます。「ガスリミット」＊「ガス価格」で算出されます。 [GWEI -> ETHの単位変換はこちらです。](https://wallet.ether1.org/helpers.html)',　/*The TX Fee is paid to miners for including your TX in a block. It is the `gas limit` * `gas price`. [You can convert GWEI -> ETHO here](https://wallet.ether1.org/helpers.html)*/
...
HELP_16_Desc_1              : 'You can use a blockchain explorer like [explorer.ether1.org](https://explorer.ether1.org/#/). Paste your address into the search bar and it will pull up your address and transaction history. For example, here\'s what our [donation account](https://explorer.ether1.org/addr/0xDECAF9CD2367cdbb726E904cD6397eDFcAe6068D) looks like onexplorer.funcoin.io ',
HELP_17_Title               : '17) Why isn\'t my balance showing up when I unlock my wallet? ',
HELP_17_Desc_1              : 'This is most likely due to the fact that you are behind a firewall. The API that we use to get the balance and convert said balance is often blocked by firewalls for whatever reason. You will still be able to send transactions, you just need to use a different method to see said balance, likeexplorer.funcoin.io ',
